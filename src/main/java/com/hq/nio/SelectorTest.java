 ...
 * @description:
 * @author: Mr.Huang
 * @create: 2019-08-16 10:59
 **/
public class SelectorTest {
    /**
     *  abstract class Selector implements Closeable
     *  Selector是一个抽象方法，实现了closeable,会自动close方法
     *  一般使用open方法
     *  Selector(选择器)是nio中能够检测一个到多个的NIO通道，
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("selectot的底层创建对象："+ SelectorProvider.provider().getClass());
        for (int i=0;i<ports.length;i++){
            ServerSocketChannel serverSocketChannel = ServerSocketChannel.open();
            //配置成不阻塞必须写
            //与Selector一起使用时，Channel必须处于非阻塞模式下。这意味着Channel与Selector不能一起使用。
